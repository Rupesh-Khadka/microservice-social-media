...
import {
  handlePostCreated,
  handlePostDeleted,
} from "./eventHandlers/searchEventHandlers";
...
app.use("/api/posts", sensitiveEndPointsLmit);

// Atching Redis to route
app.use(
  "/api",
...
async function startServer() {
  try {
    ...
    //consume the event or subscribe
    await consumeEvent("post-created", async (event) => {
      await handlePostCreated(event, redisClient);
    });
    await consumeEvent("post-delete", async (event) => {
      await handlePostDeleted(event, redisClient);
    });

    //start server
    app.listen(PORT, () => {
      logger.info(`Search service runnning on port ${PORT}`);
    });
  } catch (e) {
